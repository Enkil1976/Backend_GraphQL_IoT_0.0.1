...
const Sensor = {
  /**
   * Check if sensor is online based on recent data
   */
  isOnline: async(sensor, args, context) => {
    try {
      // For now, just return the is_active value from database
      // Later this could be enhanced to check last_seen timestamp
      return sensor.is_active !== false;
    } catch (error) {
      console.error(`[SensorTypeResolver] Error checking online status for sensor ${sensor.id}:`, error);
      return false;
    }
  },
